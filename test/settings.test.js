...
test('normalizes each setting independently', () => {
  assert.deepEqual(normalizeSettings({ fontSize: 54, mirror: true, keepAwake: true }), {
    fontSize: 54,
    mirror: true,
    keepAwake: true,
  });
...
test('loads a valid versioned settings record', () => {
  const storage = memoryStorage(
    JSON.stringify({ version: 1, fontSize: 46, mirror: true, keepAwake: false }),
  );
  assert.deepEqual(loadSettings(storage), { fontSize: 46, mirror: true, keepAwake: false });
});
...
test('saves a normalized, versioned record', () => {
  const storage = memoryStorage();
  assert.equal(saveSettings({ fontSize: 64, mirror: true, keepAwake: true }, storage), true);
